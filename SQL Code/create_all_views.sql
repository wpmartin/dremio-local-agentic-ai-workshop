...
CREATE VIEW catalog.rise.bronze.trips_raw AS SELECT * FROM Samples."samples.dremio.com"."NYC-taxi-trips.csv";
CREATE VIEW catalog.rise.bronze.nyc_weather_raw AS SELECT * FROM Samples."samples.dremio.com"."NYC-weather.csv";
CREATE VIEW catalog.rise.bronze.sf_weather_raw AS SELECT * FROM Samples."samples.dremio.com"."SF weather 2018-2019.csv";

-- Create silver-layer trips View with cleaned attribute names
CREATE VIEW catalog.rise.silver.trips AS SELECT 
...
FROM   (SELECT 
            ...
            tip_amount,
            total_amount
        FROM  catalog.rise.bronze.trips_raw AS nyc_trips
) nested_0;

-- Create silver-layer weather View with cleaned attribute names and data types
CREATE VIEW catalog.rise.silver.nyc_weather AS SELECT 
        station,
        location_name,
...
FROM   (SELECT 
        ...
        FROM   catalog.rise.bronze.nyc_weather_raw AS nyc_weather
) nested_0;

-- Create gold-layer trips View enriched with weather data
CREATE VIEW catalog.rise.gold.trips_enriched AS SELECT 
    location_name,
    pickup_date,
    pickup_time,
    passenger_count,
    ...
    temp_min
FROM 
    catalog.rise.silver.trips as t INNER JOIN catalog.rise.silver.nyc_weather as w ON t.pickup_date = w.calendar_date
    LIMIT 30000000;
